...
class RLTask(BaseTask):
    ...
    def __init__(self, name, env, offset=None) -> None:
        ...
        self.test = False # self._cfg["test"]
        self._device = 'cuda:0' # self._cfg["sim_device"]
        print("Task Device:", self._device)

        self.clip_obs =  5.0 # self._cfg["task"]["env"].get("clipObservations", np.Inf)
        self.clip_actions = 1.0 # self._cfg["task"]["env"].get("clipActions", np.Inf)
        self.rl_device = 'cuda:0' # self._cfg.get("rl_device", "cuda:0")

        self.control_frequency_inv = 2 # self._cfg["task"]["env"].get("controlFrequencyInv", 1)
